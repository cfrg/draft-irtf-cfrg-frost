...
# This file was *autogenerated* from the file test_oprf.sage
from sage.all_cmdline import *   # import sage library

_sage_const_17 = Integer(17); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_68 = Integer(68); _sage_const_2 = Integer(2)#!/usr/bin/sage
# vim: syntax=python
...
class Protocol(object):
    def __init__(self, suite, mode):
        self.inputs = [b'\x00', b'\x5A'*_sage_const_17 ]
        self.suite = suite
        self.mode = mode
        ...
        def create_test_vector_for_input(x):
            ...
            if self.mode == mode_verifiable:
                vector["EvaluationProof"] = {
                    "c": to_hex(group.serialize_scalar(proof[_sage_const_0 ])),
                    "s": to_hex(group.serialize_scalar(proof[_sage_const_1 ])),
                }

            vector["Input"] = to_hex(x)
            vector["Info"] = to_hex(info)
            vector["Output"] = to_hex(output)
            vector["Batch"] = int(_sage_const_1 )
...
def write_base_vector(fh, vector):
    ...
    for i, v in enumerate(vector["vectors"]):
        fh.write("#### Test Vector " + str(i+_sage_const_1 ) + ", Batch Size " + str(v["Batch"]) + "\n")
        fh.write("\n")
        fh.write("~~~\n")
...
def main(path="vectors"):
    ...
    with open(path + "/allVectors.json", 'wt') as f:
        json.dump(flatVectors, f, sort_keys=True, indent=_sage_const_2 )
        f.write("\n")
